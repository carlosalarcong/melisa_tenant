$(document).ready(function() {
	...
	$("input[type=number]").bind('keyup input', function() {
		...
		var subTotal = 0;
		precio = $(this).closest('tr').find('.valor-articulo').text();
		cantidad = $(this).closest('tr').find('.cantidad-articulo').val();
		subTotal += precio * cantidad;
		$(this).closest('tr').find('.subtotal-articulo').text(subTotal);

	});

	$('input.cantidad-articulo').keypress(function(e) {
		return false;
	});

	$('#mostrar-formulario-pagos').click(function(e) {
		...
		var totalCuenta = $('.total-cuenta').data('totalCuenta');


		if (parseInt(totalCuenta) === 0) {

			var dinamicoEfectivo = $('.localizacion-input-efectivo').find('input').data('dinamicoEfectivo');
			$('#rebsol_hermesbundle_MediosPagoType_monto_' + dinamicoEfectivo + '_0').val(0);

			showTablas(dinamicoEfectivo, 1);

			$('#sumaTotalMediosDePago').text(0);
			$('#SaldoTotalMediosDePago').text(0);
			$('#despues-agregar-botones').removeClass('hide');
			$('#despues-agregar-botones').appendTo('#agregar-botones');
			$('#regresar-datos').addClass('btn-mini');
			...
			$.ajax({
				...
				data: {
					idPaciente: $('#mostrar-formulario-pagos').data('idPaciente'),
					pagoCuentaFarmacia: $('.total-cuenta').data('totalCuenta'),
					formulariotypeCantidadArticulos: $('.formulario-cantidad-articulo').serializeArray(),
					idPrevision: $("#rebsol_hermesbundle_PrestacionType_prevision").val(),
					idConvenio: $("#rebsol_hermesbundle_PrestacionType_convenio").val()
				},
				dataType: 'json'
			});
			...
			<strong class='green'>Generando Pago, </strong> <br>
			Por favor espere...
			</div>`;
			...
			$("#realizar-pago-articulos").slideDown(1600);

			$('#sumaTotalMediosDePago').text(parseInt($('.total-cuenta').data('totalCuenta')));
			$('#SaldoTotalMediosDePago').text(parseInt($('.total-cuenta').data('totalCuenta')));
			$('#despues-agregar-botones').removeClass('hide');
			$('#despues-agregar-botones').appendTo('#agregar-botones');
			$('#regresar-datos').addClass('btn-mini');
			$('#btn-pagar').addClass('btn-mini');
	...
	$('#regresar-datos').click(function(e) {

		e.preventDefault();
		$('.cancelar-desde-mediopago').remove();
		$('.realizar-pago-cuenta').remove();
		$("#ocultar-detalle-receta").slideDown(1600);
